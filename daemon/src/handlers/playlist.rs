...
use crate::{
    helpers::{add_songs_to_playlist, create_playlist, delete_playlist, remove_song_from_playlist, rename_playlist, send_to_client},
    types::*,
};
use aurora_protocol::{PlaylistIn, Response};

pub async fn playlist_rename(stream: &WriteSocket, state: &State, playlist_id: Uuid, new_title: String) -> anyhow::Result<()> {
    match rename_playlist(playlist_id, new_title).await {
        Ok(_) => playlist_list(stream, state).await,
        Err(err) => {
...
pub async fn playlist_delete(stream: &WriteSocket, state: &State, playlist_id: Uuid) -> anyhow::Result<()> {
...
pub async fn playlist_create(stream: &WriteSocket, state: &State, item: PlaylistIn) -> anyhow::Result<()> {
    match create_playlist(item).await {
        Ok(name) => {
            tracing::info!("Created playlist {name}.");
            playlist_list(stream, state).await?;
        }
        Err(err) => {
            send_to_client(
                stream,
...
pub async fn playlist_list(stream: &WriteSocket, state: &State) -> anyhow::Result<()> {
    let mut state = state.lock().await;
    match state.get_all_playlists().await {
        Ok(list) => {
...
pub async fn playlist_add_songs(
    ...
    let result = add_songs_to_playlist(&state_locked, playlist_id, song_ids).await;
    drop(state_locked);
    match result {
        Ok(_) => {
            playlist_list(stream, state).await?;
