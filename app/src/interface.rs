    ...
    let _ = app.upgrade_in_event_loop(move |aurora: AuroraPlayer| {
        ...
        aurora.on_remove_song_from_playlist(move |playlist_id_str, song_id_str| {
            ...
            tokio::spawn(async move {
                let (ids, writer_tx) = {
                    let state_locked = state.lock().await;
                    let ids: Vec<String> = if state_locked
                        .selected_song_ids
                        .contains(song_id_str.as_str())
                    {
                ...
                if let Ok(playlist_id) = uuid::Uuid::parse_str(&playlist_id_str) {
                    for id_str in &ids {
                        if let Ok(song_id) = uuid::Uuid::parse_str(id_str) {
                            let _ = writer_tx
                                .send(Request::PlaylistRemoveSong {
                                    playlist_id,
                                    song_id,
                                })
                                .await;
        ...
        aurora.on_add_songs_to_playlist(move |playlist_id_str, primary_song_id| {
            ...
            tokio::spawn(async move {
                let (song_ids, writer_tx) = {
                    let state_locked = state.lock().await;
                    let song_ids: Vec<uuid::Uuid> = if state_locked
                        .selected_song_ids
                        .contains(primary_song_id.as_str())
                    {
                        state_locked
                            .selected_song_ids
                            .iter()
        ...
        aurora.on_set_repeat(move |r| {
            ...
            tokio::spawn(async move {
                let _ = state
                    ...
                    .await
                    .writer_tx
                    .send(Request::SetRepeat(r as u8))
                    .await;
            });
        });
        ...
        aurora.on_get_songs_by_artist(move |artist| {
            ...
            tokio::spawn(async move {
                ...
                let _ = state_locked
                    .writer_tx
                    .send(Request::Search(aurora_protocol::SearchType::ByArtist(
                        artist_str,
                    )))
                    .await;
            });
